...
if __name__ == '__main__':
    ...
    for t in t_array:
        ...
        c_kt_sigma = localVolMC(locvol = locvolCEV, S0 = S0, T = t,  paths = paths, timeSteps = timesteps, AK = k_array)
        c_kt_s_sigma_perturb = localVolMC(locvol = lambda u, t: locvolCEV(u,t) + delta_sigma, S0 = S0 + delta_S, T = t,  paths = paths, timeSteps = timesteps, AK = k_array)
        c_kt_sigmaperturb = localVolMC(locvol = lambda u, t: locvolCEV(u,t) - delta_sigma, S0 = S0, T = t,  paths = paths, timeSteps = timesteps, AK = k_array)
        c_kt_s_perturb = localVolMC(locvol = locvolCEV, S0 = S0 + delta_S, T = t,  paths = paths, timeSteps = timesteps, AK = k_array)
    ...
    for t in t_array:
        ...
        c_kt_sigma_x_vanna = localVolMC(locvol = lambda u, t: locvolCEV(u,t) + x_vanna_kt(u,t,grid=False).flatten(), S0 = S0, T = t,  paths = paths, timeSteps = timesteps, AK = k_array, debug = False) 
        c_kt_sigma_x_volga = localVolMC(locvol = lambda u, t: locvolCEV(u,t) + x_volga_kt(u,t,grid=False).flatten(), S0 = S0, T = t,  paths = paths, timeSteps = timesteps, AK = k_array) 
